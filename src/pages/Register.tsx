...
export default function Register({ onSwitch }: Props) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    catch (err: any) { setError(err.response?.data?.error || 'Kayıt başarısız'); }
      ...
      <div className="bg-[#2b2d31] rounded-lg p-8 w-full max-w-md shadow-2xl">
        ...
        <form onSubmit={submit} className="flex flex-col gap-4">
          <div>
            ...
            <input value={username} onChange={e => setUsername(e.target.value)} type="text" required minLength={2}
        ...
        <p className="text-[#949ba4] text-sm mt-4 text-center">
          ...
          <button onClick={onSwitch} className="text-[#5865f2] hover:underline">Giriş yap</button>
        </p>
      </div>
    </div>
  );
